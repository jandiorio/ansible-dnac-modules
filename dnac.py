...
class DNACModule(object):
    ...
    def login(self):
        ...
        url = '%(protocol)s://%(hostname)s/api/system/v1/auth/login/' % self.params
        ...
        # Handle DNAC response
        if auth['status'] != 200:
            self.result['response'] = auth['msg']
            self.result['status'] = auth['status']
            try:
                # DNAC error
                dnac_response_json(self.result, auth['body'])
    ...
    def request(self, path, payload=None):
        ...
        if info['status'] != 200:
            try:
                # DNAC error
                dnac_response_json(self.result, info['body'])
        ...
        dnac_response_json(self.result, resp.read())
